...
def test_list_instances_includes_instance_config_summary(tmp_path: Path):
    ...
    assert payload["instanceConfig"] == {
        ...
        "contextEvictRatio": None,
        "contextCompressionMaxChars": None,
...
def test_status_websocket_includes_instance_details(tmp_path: Path):
    ...
    assert payload["data"]["cpuUsage"] == payload["data"]["systemCpuUsage"]
    assert payload["data"]["memoryUsage"] == payload["data"]["processMemoryMb"]
    assert payload["data"]["systemMemoryUsage"] >= 0
    assert payload["data"]["systemMemoryUsedMb"] >= 0
    assert payload["data"]["systemMemoryTotalMb"] >= 0
    assert payload["data"]["processMemoryMb"] >= 0
