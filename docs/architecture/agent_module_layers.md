...
| Utils | `shinbot/agent/utils/` | 可复用、无连续会话语义的纯计算能力，例如消息窗口整理、结构化转换、token 估算 | 控制状态迁移、长期持有 session 状态、隐式触发回复、执行 LLM 压缩 |
| Runtime / Services | `shinbot/agent/runtime/`、`shinbot/agent/services/` | ModelRuntime、PromptRegistry、ToolManager、ContextBuilder、数据库适配器等基础服务 | 知道 review/active 的业务策略 |
...
Coordinator 需要读取或写入外部数据时，应定义自身需要的 port/protocol。例如 review 的 `ReviewMessageStore`、`ReviewSummaryStore` 属于 review coordinator 的输入输出边界；SQLite 等具体实现放在 runtime adapter 中。
...
- `ReviewScanStageRunner`
  - 输入一批 review 消息与压缩摘要；
  - 输出有回复价值的消息 id。
- `CompressionStageRunner`
  - 输入被上游选定的消息或上下文块；
  - 输出摘要、覆盖范围、候选消息 id 和理由；
  - 不决定哪些消息应该被压缩。
...
Utils 是可复用的一次性纯能力。它不具有连续性、不拥有 tool loop、不维护状态机。涉及 LLM 调用的单次 stage 应优先命名为 runner，例如 review compression 或未来的 context eviction compression。
...
`context` 的现行职责边界以 `agent_context_boundary.md` 为准：它只做上下文结构化、投影、打包和自身轻量状态管理，不负责消息选择、workflow 触发或压缩摘要生成。
...
- `coordinators/review/stores.py`：review 所需 message/summary store port。
- `runtime/review_stores.py`：review store 的 SQLite/database adapter。
