...
def test_load_provider_schema_merges_adjacent_i18n_file(tmp_path: Path) -> None:
    schema_path = write_provider_files(tmp_path)
    (tmp_path / "config.i18n.zh-CN.toml").write_text(
        """
[provider]
display_name = "OneBot v11"
description = "OneBot v11 消息平台适配器。"

[[fields]]
path = "mode"
label = "连接模式"
description = "WebSocket 连接方式。"
choices = { forward = "正向连接", reverse = "反向监听" }

[[fields]]
path = "reverse.port"
label = "反向监听端口"
description = "反向模式监听端口。"
""".strip()
        + "\n",
        encoding="utf-8",
    )

    provider = load_provider_schema(schema_path)

    assert provider.metadata["i18n"]["zh-CN"] == {
        "display_name": "OneBot v11",
        "description": "OneBot v11 消息平台适配器。",
    }
    assert provider.fields[0].metadata["i18n"]["zh-CN"] == {
        "label": "连接模式",
        "description": "WebSocket 连接方式。",
        "choices": {"forward": "正向连接", "reverse": "反向监听"},
    }
    assert provider.fields[1].metadata["i18n"]["zh-CN"]["label"] == "反向监听端口"
