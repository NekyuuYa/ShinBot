...
- `trace_ids`: 一个批处理模型调用覆盖多条 ingress trace 时的追踪 ID 列表；同时保留首个 `trace_id` 便于 WebUI 快速过滤。
...
- `agent.active_chat.message.observed`: active chat 观察到消息并更新短期 attention，属于逐消息 DEBUG。
- `agent.active_chat.round.started` / `agent.active_chat.round.finished`: active chat 批次进入模型 round 与完成，包含 message ids、action、interest delta 和 trace。
- `agent.active_chat.round.restored` / `agent.active_chat.round.failed`: active chat 批次失败后恢复 pending 或异常退出。
- `agent.active_chat.semantic_wait.started` / `agent.active_chat.semantic_wait.flushed`: active chat 语义等待计时器的 DEBUG 生命周期。
...
- `agent.review.workflow.failed`: review workflow 异常并进入保底 active chat。
- `agent.review.consume.failed`: review 已处理区间写回失败。
- `agent.review.bootstrap.failed` / `agent.review.bootstrap.task_failed`: active chat bootstrap 后台阶段失败。
- `agent.review.summary.persist_failed`: review summary 持久化失败。
- `agent.active_chat.workflow.start` / `agent.active_chat.workflow.message`: active chat workflow 启动和消息通知。
- `agent.active_chat.prompt_build.failed` / `agent.active_chat.model_call.failed`: active chat prompt 构造或模型调用失败，只记录 IDs、阶段和异常类型，不记录 prompt 或模型正文。
- `agent.idle_review_planning.prepared`: scheduler 在 session mutex 内冻结一次即将退出 active chat 的快照；后续模型调用不持有该 mutex。
- `agent.idle_review_planning.start` / `agent.idle_review_planning.finish`: ACTIVE_CHAT 退出 IDLE 前由 LLM 生成下一次 review 间隔。`finish` 包含原始/限幅后间隔、model execution id 与 prompt signature。
- `agent.idle_review_planning.fallback` / `agent.idle_review_planning.failed`: planner 不可用、模型失败、输出无延迟或无效时回退到静态 review policy。
- `agent.idle_review_planning.discarded`: 规划结果在提交前因 state、active-chat 快照或 review plan 已变化而被拒绝，不会覆盖较新的会话状态。
- `agent.runtime.idle_review_planning.deferred` / `coalesced` / `superseded`: runtime 将模型 I/O 移出 session mutex 后的本地请求生命周期；相同快照的重复 timer 信号合并，较新的退出意图会使旧结果失效。
- `audit_logs` 中的 `agent.idle_review_planning.model_result` 与 `agent.idle_review_planning.application`: 每个冻结请求的模型阶段和最终应用阶段均以 `signal_id` 关联。记录 trigger、epoch、结果类别、错误码、model execution id、prompt signature、候选与实际 plan；不保存 prompt 或模型正文。
- `/api/v1/agent-runtime` 将最近的同类审计按 `signalId` 配对为 `idleReviewPlanningDecisions`，展示模型结果、fallback/stale discard 与最终计划。该投影只返回白名单字段，且 `latestAudit` 仅是无 metadata 的摘要。
...
`trace_id` 从消息入口生成，经过 route target、AgentSignal、AgentScheduler unread snapshot，继续进入 review stage input、active chat batch 和 `ModelRuntimeCall.metadata`。因此一次真实消息触发的 review scan、reply decision、active chat fast mode 等模型调用，可以在模型审计页面和日志页面按同一 trace 关联。

有些数据来自历史数据库重建，例如启动后从 `message_logs` 恢复的旧 unread 范围。当前数据库不把 trace 作为 unread 持久字段，这类历史上下文允许没有 `trace_id`。排障时回退到 `message_log_id` 或 `session_id`。

## Legacy 边界

本阶段优先结构化 Agent workflow、模型调用、adapter 生命周期、timer 与后台任务的高价值日志。仍可能保留少量开发工具、debug plugin、权限绑定、插件加载和第三方库日志，它们不在状态机主链路上，或者由专门插件面板观察。后续触碰这些模块时再就近转换为 `format_log_event`。
