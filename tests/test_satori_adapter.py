...
class TestAsyncIngressQueue:
    @pytest.mark.asyncio
    async def test_handle_event_uses_worker_queue_when_available(self):
        adapter = _make_adapter()
        received: list[UnifiedEvent] = []
        done = asyncio.Event()
    ...
    def test_enqueue_event_drops_oldest_when_queue_is_full(self):
        adapter = _make_adapter()
