...
| Workflow | `shinbot/agent/workflows/` | 处理一个明确输入并执行一段完整工作流，例如聊天 runtime 的 tool loop、回复决策、专用 LLM 决策流程 | 决定何时被触发、维护全局状态机、扫描 unread |
| Utils | `shinbot/agent/utils/` | 可复用、无连续会话语义的能力，例如上下文压缩、图片转述、消息窗口整理、结构化转换 | 控制状态迁移、长期持有 session 状态、隐式触发回复 |
| Runtime / Services | `shinbot/agent/runtime/` 等 | ModelRuntime、PromptRegistry、ToolManager、ContextBuilder、消息存储访问等基础服务 | 知道 review/active 的业务策略 |
...
- `ReviewCoordinator`
  ...
  - 调用 review scan workflow 筛选值得回应的消息；
  - 调用 reply decision workflow 决定回复；
  - 调用 active bootstrap workflow 生成 active chat 初始语义档位；
...
- `FastChatWorkflow`
  ...
  - 返回已消费消息、tool 行为、interest delta、是否强制退出等结构化结果。
- `ReviewScanWorkflow`
  - 输入一批 review 消息与压缩摘要；
  - 输出有回复价值的消息 id。
- `ReplyDecisionWorkflow`
  - 输入核心回复目标和附近上下文；
...
Tool 定义和可见性由 ToolRegistry / ToolManager 管理。模型请求中的 `tools` 数组应由对应 workflow 根据当前任务向 ToolManager 获取。
