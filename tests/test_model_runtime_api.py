...
def test_provider_capability_type_stored_and_returned(tmp_path: Path):
    ...
    with TestClient(app) as client:
        for capability_type in ("completion", "embedding", "rerank", "tts", "stt", "image", "video"):
            resp = client.post(
                "/api/v1/model-runtime/providers",
                headers=headers,
                json={
        ...
        assert returned_types == {"completion", "embedding", "rerank", "tts", "stt", "image", "video"}
...
def test_provider_probe_embedding_type_uses_embed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    with TestClient(app) as client:
        ...
        async def fake_embed(call):
            return type("FakeResult", (), {"execution_id": "embed-probe-exec", "embedding": [0.1]})()
