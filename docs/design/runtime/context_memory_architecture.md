...
PromptRegistry 仍然遵循 `prompt_registry.md` 中定义的七阶段顺序；本设计只重新定义 Stage 4 的内部结构，以及 Stage 6 与上下文系统的边界。
...
- 运行时淘汰应优先作用于 `SealedBlockDeque` 的队首，避免因为缓存压力直接扰动最新 `OpenBlock`。
- 若没有任何 sealed block 但 token 压力已经触发，可以把 `OpenBlock` 作为应急 fallback 淘汰，防止上下文无法收缩。
...
- `CompressedMemory`
  - 从一批被淘汰的短期块中提炼出的压缩摘要
  - 用于后续快速回忆
  - 运行时由 `CompressedMemoryProjector` 投影为压缩记忆消息与压缩源文本
...
- 由压缩器基于淘汰候选生成
- 可进一步合并，但不回写原始短期块
- 压缩源文本应在投影阶段展开临时 alias，避免把 `P0/A0` 这类 Prompt 局部代称固化进中期记忆
...
- `LongTermMemoryProvider` 负责按 prompt request 检索候选长期记忆。
- `NoopLongTermMemoryProvider` 默认返回空列表，保证行为不变。
- `LongTermMemoryProjector` 负责将候选投影为长期记忆 context message。
- `PromptMemoryAssembler` 将长期记忆消息放在 context messages 最前缀，为后续接入语义检索预留稳定位置。
...
- **Alias Projection**
  - 当前 Prompt 中采用何种简称
  - 仅影响渲染，不影响长期事实本体
  - 运行时由 `AliasContextProjector` 负责投影为 inactive alias context message 与 active alias constraint
...
- 负责将连续同 prompt role 的记录归并为 `TimelineRun`。
...
运行时应通过 `PromptMemoryAssembler` 完成最终组装：

- 它只编排投影片段顺序，不拥有记忆事实。
- 它调用 runtime/projector 暴露的窄接口获取 context、instruction、alias constraint。
- 它是后续插入长期记忆投影的稳定入口。
